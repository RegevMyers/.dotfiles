...
if [ "$color_prompt" = yes ]; then
  PROMPT_COMMAND='
    ...
    PS1="${debian_chroot:+($debian_chroot) }\[\033[0m\033[0;32m\]\u\[\033[2;3;32m\]@\h\[\033[0m\] $__MY_GIT_PS1 \$ "
  '
else 
...
if [ -z "$TMUX" ]; then
  ...
  if [[ -n "$SSH_TTY" || "$(uname -n)" = *"-pi" ]]; then
    ...
    echo "[ @ ] Starting tmux"
    exec tmux new -A -s tmux -n main -c $HOME "echo; fastfetch; poem --book 'Dao De Jing'; echo; $SHELL"
  fi
fi
